...
export class Monde {
  ...
  // CE QU'UN CHANTIER ÉLOIGNÉ REÇOIT EN UN MOIS, en fraction de sa commande.
  //
  // C'était un INTERRUPTEUR : pendant dix mois un chantier de colonie ne recevait
  // rien du tout, puis tout d'un coup. Le joueur voyait une barre de progression
  // figée à zéro sans savoir si son chantier était en route ou mort, et rien ne
  // distinguait « le convoi arrive » de « il n'y a plus de planches ».
  //
  // C'est devenu une CADENCE. Le chantier se remplit dès le premier mois, à un
  // dixième de sa commande — la mule, pas le train. Il met le même temps, mais on
  // le voit avancer, et l'on voit du même coup s'il n'avance pas.
  //
  // Une ville de la carte est chez elle et reçoit à plein. Une colonie reliée par
  // le rail aussi : c'est très exactement ce que la voie lui apporte.
  cadenceLivraison(ch, ville) {
    void ch;
    if (ville.fondee === undefined) return 1;
    if (ville.marche && ville.marche.villes.length > 1) return 1;
    return 1 / P.moisAcheminement;
  }

  // Ce qu'un chantier peut demander ce mois-ci, ressource par ressource : sa
  // cadence appliquée à sa commande d'origine, plafonnée par ce qu'il lui reste.
  tranche(ch, ville) {
    const cad = this.cadenceLivraison(ch, ville);
    const commande = materiaux(ch.type);
    const out = {};
    for (const [r, reste] of Object.entries(ch.restant)) {
      if (reste <= 0) continue;
      out[r] = Math.min(reste, (commande[r] || 0) * cad);
    }
    return out;
  }
  ...
  tick() {
    ...
    for (const v of this.villes) {
      ...
      for (const ch of this.tousChantiers(v)) {
        ...
        for (const [r, q] of Object.entries(this.tranche(ch, v))) {
          if (q > 0) mf.demander(r, q, mf === m ? v : null);
        }
      }
        ...
        for (const v of this.villes) {
          ...
          for (const ch of chs) {
            ...
            for (const [r, q] of Object.entries(this.tranche(ch, v))) {
              ...
              ch.restant[r] = Math.max(0, ch.restant[r] - servi);
