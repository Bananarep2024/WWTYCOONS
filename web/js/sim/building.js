...
export class Batiment {
  ...
  // Ce que vaut le bâtiment, à l'achat comme à la vente.
  //
  // Une affaire vaut le meilleur de ses deux destins : ce qu'elle rapporte si on
  // la garde, ou ce que valent ses murs si on l'arrête. Prendre le maximum des
  // deux est la seule formule qui ne casse ni dans un sens ni dans l'autre.
  //
  // Le plancher à zéro qu'il y avait ici fermait bien l'arbitrage — acheter à
  // perte, démolir, revendre le terrain plus cher — mais au prix d'une aberration
  // plus grosse : dès que le profit passait sous zéro, les murs ne valaient plus
  // RIEN. On rachetait une aciérie à quatre cases pour le prix de son sol, soit
  // 3 240 $ de bâti offerts.
  //
  // L'arbitrage reste fermé, et plus largement qu'avant : on paie désormais
  // terrain + résiduelle, on ne récupère que le terrain en démolissant, donc
  // l'opération perd exactement la valeur résiduelle.
  get valeurDeCession() {
    return this.terrainCourant + Math.max(P.valeurResiduelle * this.valeurBatie,
                                          P.anneesDeProfit * this.profitAnnuel);
  }
