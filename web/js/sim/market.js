...
export class Marche {
  ...
  // réellement EMPORTÉ, et c'était faux — non parce que le matelas s'achète (il
  // ne s'achète pas : c'est un terme de prix, pas une commande), mais à cause du
  // service local. Une ville exportatrice garde dans son livre ce qu'elle
  // destine à la voisine ; tant que la voisine n'est pas venue, la marchandise
  // n'est prélevée nulle part. Son producteur se croyait invendu, ralentissait,
  // et la contraction remontait la filière : les cinq villes tombaient de 600 à
  // 120 ménages. Les besoins DÉCLARÉS ignorent cette géographie, et c'est
  ...
  // LA TENSION NUE, celle que l'écoulement borne à 1.
  //
  // Un atelier ne peut pas se régler sur le taux d'écoulement seul : ce taux
  // porte sur l'offre DÉJÀ BRIDÉE, si bien que le bridage se mesure lui-même.
  // Viser l'écoulement fait converger la mémoire vers √(demande ÷ capacité) au
  // lieu de la demande — pour 200 demandés sur 1 000 de capacité, l'atelier se
  // stabilise à 447 et jette 247 unités par mois, indéfiniment. Il faut viser
  // la part de CAPACITÉ qui a trouvé preneur, `débouché × tension nue`, dont le
  // seul point fixe est l'invendu nul. Non bornée, elle dit aussi de combien on
  // MANQUE, et c'est par là qu'un atelier en sommeil rouvre.
  tensionEcoulement(res) {
    const offert = this.entrees[res];
    if (offert <= 0.0001) return 1;
    return this.besoins[res] / offert;
  }
